...
class BaseStationApp:
    ...
    def setup_ui(self):
        ...
        labels = ["Liczba kanałów", "Długość kolejki", "Natężenie ruchu [lambda]",
                  ...
                  "Minimalna długość rozmowy", "Maksymalna długość rozmowy", "Czas symulacji"]
        self.inputs = {}
    ...
    def sim_loop(self):
        p = {
            ...
            'min': float(self.inputs["Minimalna długość rozmowy"].get()),
            'max': float(self.inputs["Maksymalna długość rozmowy"].get()),
    ...
    def update_plots(self):
        ...
        self.ax_q.set_title("Średnia długość kolejki (Q)")
        ...
        self.ax_ro.set_title("Intensywność ruchu (Ro)")
    ...
    def save_to_file(self, params):
        filename = f"wyniki_symulacji_{int(time.time())}.txt"
        try:
            with open(filename, "w", encoding="utf-8") as f:
                ...
                f.write("\n=== WYNIKI (KOLUMNY) ===\n")
                # Nagłówki kolumn: Czas, Kolejka (Q), Oczekiwanie (W), Natężenie (Ro)
                f.write("Czas[s]\tQ\tW\tRo\n")

                # Zapisywanie danych z historii
                for i in range(len(self.history['time'])):
                    t = self.history['time'][i]
                    q = self.history['Q'][i]
                    w = self.history['W'][i]
                    ro = self.history['Ro'][i]
                    f.write(f"{t}\t{q}\t{w:.2f}\t{ro:.2f}\n")
